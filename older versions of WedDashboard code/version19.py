    ...
    html.Div(style={
        ...
        html.Div(style={
            ...
            dcc.Link(html.Div([
                ...
                html.P("Monthly Moisture Analysis")
            ], style={
                'backgroundColor': '#ffffff',
                'padding': '20px',
                'width': '200px',
...
def create_sheet_layout(sheet_name, id_prefix):
    return html.Div(style={'backgroundColor': '#f9f9f9', 'font-family': 'Arial'}, children=[
        dcc.Link('Back to Home', href='/main-dashboard', style={'display': 'block', 'textAlign': 'center', 'padding': '20px', 'fontSize': '20px', 'color': '#4b0082', 'textDecoration': 'none', 'fontWeight': 'bold'}),
        html.H2(f'{sheet_name}', style={'textAlign': 'center', 'padding': '20px', 'backgroundColor': '#50e3c2', 'color': 'white', 'borderRadius': '8px', 'margin': '20px auto', 'width': '90%', 'boxShadow': '0 4px 8px rgba(0,0,0,0.1)'}),
...
def update_shutdown_jobs(*clicked_buttons):
    ...
    for i, month in enumerate(months_clicked):
        ...
        fig.add_trace(go.Bar(
            ...
            marker_color='#1f77b4', text=month_data['No. of Jobs hold'], textposition='auto'
        ))
        fig.update_layout(
...
def update_vibration_monitoring(*clicked_buttons):
    ...
    for i, month in enumerate(months_clicked):
        ...
        fig.update_layout(
            ...
            legend=dict(x=1.05, y=1), # Adjusted legend position slightly
            margin=dict(l=40, r=40, t=40, b=40),
            font=dict(family="Arial", size=12, color="#7f7f7f")
        )
...
def update_lube_oil_analysis(*clicked_buttons):
    ...
    for i, month in enumerate(months_clicked):
        ...
        fig1.add_trace(go.Bar(
            ...
            marker_color='#1f77b4', text=month_data['No. Scheduled for Analysis'], textposition='auto'
        ))
        fig1.add_trace(go.Bar(
            x=month_data['Plant'], y=month_data['No Samples analysed'], name='Samples Analysed',
            marker_color='#ff7f0e', text=month_data['No Samples analysed'], textposition='auto'
        ))
        fig1.update_layout(
            title=f'Lube Oil Analysis: Scheduled vs Analysed Samples ({month})',
            barmode='group',
            height=350,
            width=500,
            plot_bgcolor='#f9f9f9',
            font=dict(family="Arial", size=12, color="#7f7f7f"),
            margin=dict(l=40, r=40, t=40, b=40),
            legend=dict(x=0.01, y=0.99, xanchor='left', yanchor='top') # Specific legend position
        )
...
def update_spares_management(*clicked_buttons):
    ...
    for i, month in enumerate(months_clicked):
        ...
        fig.add_trace(go.Bar(
            ...
            marker_color='#6dd5ed', text=month_data['Indents Scheduled for Review in this month'], textposition='auto'
        ))
        fig.add_trace(go.Bar(
            x=month_data['Plant'], y=month_data['No. of PR Raised in this month'], name='PR Raised (This Month)',
            marker_color='#2193b0', text=month_data['No. of PR Raised in this month'], textposition='auto'
        ))
        ...
        fig.add_trace(go.Scatter(
            ...
            mode='lines+markers', name='Cumulative Indents (FY 24-25)', marker_color='#ff6f00',
        ...
        fig.add_trace(go.Scatter(
            ...
            mode='lines+markers', name='Cumulative PR Raised (FY 24-25)', marker_color='#9467bd',
            text=month_data['Cumul. No. of PR Raised till this month in F.Y. 24-25'], textposition='top center'
        ))

        fig.update_layout(
            title=f'Spares Management KPIs ({month})',
            barmode='group',
            height=500, # Increased height to accommodate legend below
            width=750, 
            plot_bgcolor='#f9f9f9',
            font=dict(family="Arial", size=12, color="#7f7f7f"),
            margin=dict(l=40, r=40, t=40, b=40),
...
def update_running_contracts(*clicked_buttons):
    ...
    for i, month in enumerate(months_clicked):
        ...
        fig.add_trace(go.Bar(
            ...
            marker_color='#2ca02c', text=month_data['Total Current Running Contracts'], textposition='auto'
        ))
        ...
        fig.add_trace(go.Bar(
            ...
            marker_color='#d62728', text=month_data['Contracts requiring Renewal (& expiring in this/next six months)'], textposition='auto' # Cleaned name
        ))
        fig.add_trace(go.Bar(
            x=month_data['Plant'], y=month_data['Contracts Expired in this month (& not requiring renewal)'], name='Contracts Expired (Not Renewed)',
            marker_color='#9467bd', text=month_data['Contracts Expired in this month (& not requiring renewal)'], textposition='auto'
        ))

        fig.update_layout(
            title=f'Running Contracts Status ({month})',
            barmode='group',
            height=420, # Increased height to accommodate legend below
            width=700, 
            plot_bgcolor='#f9f9f9',
            font=dict(family="Arial", size=12, color="#7f7f7f"),
            margin=dict(l=40, r=40, t=40, b=40),
...
def update_gate_pass_details(*clicked_buttons):
    ...
    for i, month in enumerate(months_clicked):
        ...
        fig.add_trace(go.Bar(
            ...
            marker_color='#17becf', text=month_data['New Gate Pass Created in this month'], textposition='auto'
        ))
        fig.add_trace(go.Bar(
            x=month_data['plant'], y=month_data['Gate Pass open till last month'], name='Open from Last Month',
            marker_color='#8c564b', text=month_data['Gate Pass open till last month'], textposition='auto'
        ))
        fig.add_trace(go.Bar(
            x=month_data['plant'], y=month_data['Gate Pass Closed in this month'], name='Closed this Month',
            marker_color='#e377c2', text=month_data['Gate Pass Closed in this month'], textposition='auto'
        ))
        fig.add_trace(go.Bar(
            x=month_data['plant'], y=month_data['Total current open Gate Pass'], name='Total Open Current',
            marker_color='#7f7f7f', text=month_data['Total current open Gate Pass'], textposition='auto'
        ))

        fig.update_layout(
            title=f'Gate Pass Details ({month})',
            barmode='group',
            height=420, # Increased height to accommodate legend below
            width=700, 
            plot_bgcolor='#f9f9f9',
